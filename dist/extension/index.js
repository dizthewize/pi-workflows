...
export default function piWorkflowsExtension(pi) {
    ...
    pi.events.on("workflows:plan:request", async (data) => {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Failed to read PRD: ${err}`);
            }
            const roles = loadRoles();
            const plannerRole = roles.find((r) => r.id === "planner");
            ...
            if (subagentFn) {
                try {
                    ...
                    output = "Planner execution failed.";
                }
            }
            else {
    ...
    pi.events.on("workflows:execute:request", async (data) => {
        ...
        try {
            ...
            let spawnFn = undefined;
            if (hasRoles) {
                spawnFn = async (spawnOpts) => {
                    const roleId = spawnOpts.agent?.name ?? "worker";
                    return new Promise((resolve, reject) => {
                        const rid = randomUUID();
                        const unsub = pi.events.on(`roles:dispatch:response:${rid}`, (res) => {
                            unsub();
                            if (res.success) {
  ...
  execute_workflow({
    ...
    options: { maxParallel: 4, maxCost: 15 }
                ...
                onUpdate: (update) => {
                    if (onUpdate) {
                        onUpdate({
                            ...
                            details: update.details?.results.reduce((acc, r) => {
                                acc[r.agent] = {
                                    output: r.output,
                                    exitCode: r.exitCode,
                                    cost: r.usage?.cost,
        ...
        renderCall(args, theme) {
            ...
            const depCount = (args.tasks ?? []).reduce((sum, t) => sum + (t.dependsOn?.length ?? 0), 0);
