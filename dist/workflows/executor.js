...
function readRolesStore() {
    try {
        return JSON.parse(fsSync.readFileSync(ROLE_STORE_PATH, "utf-8"));
    }
    catch {
        return [];
    }
...
function meshJoinIfRequested(opts, name) {
...
function meshBroadcast(message) {
    ...
    fsSync.writeFileSync(path.join(inboxDir, `${Date.now()}-${msg.id}.json`), JSON.stringify(msg, null, 2));
}
function meshLeave(agentId) {
    const registryFile = path.join(MESH_DIR, "registry", `${agentId}.json`);
    try {
        fsSync.unlinkSync(registryFile);
    }
    catch { /* ignore */ }
}
...
import { generateLogMarkdown } from "../utils.js";
...
export async function executeWorkflow(opts) {
    ...
    fsSync.writeFileSync(initialStatePath, JSON.stringify({ name, tasks, options, waveCount: waves.length }, null, 2), "utf-8");
    ...
    for (const wave of waves) {
        ...
        const results = new Array(wave.tasks.length);
        ...
        for (let i = 0; i < wave.tasks.length; i++) {
            if (running.size >= maxParallel) {
                ...
                const [idx, settled] = await Promise.race([...running.entries()].map(([k, p]) => p.then((r) => [k, r])));
                running.delete(idx);
                results[idx] = settled;
                costTracker.add(settled.usage?.cost ?? 0, `task-${wave.tasks[idx].id}`);
                if (onUpdate)
                    onUpdate(createWaveUpdate(wave, results));
            }
            const taskIdx = i;
        ...
        // Wait for remaining
        if (running.size > 0) {
            const settled = await Promise.all([...running.entries()].map(([k, p]) => p.then((r) => [k, r])));
        ...
        for (let i = 0; i < wave.tasks.length; i++) {
            ...
            if (r.output) {
                try {
                    ...
                    fsSync.writeFileSync(path.join(workflowDir, "results", t.id, "output.md"), r.output, "utf-8");
                }
                catch { }
            }
        }
    ...
    if (options.saveLog !== false) {
        result.logPath = path.join(runtime.cwd, `workflow-log-${name}-${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}.md`);
...
function createWaveUpdate(wave, results) {
    return {
        mode: "single",
        agentScope: "user",
...
function buildReviewerPrompt(task, result) {
    return `
...
async function retryWithFeedback(task, previousResult, feedback, team, workflowDir, runtime, signal, spawnFn) {
    let agentCfg;
    if (task.roleId) {
        const roleCfg = resolveRoleConfig(task.roleId);
        agentCfg = roleCfg ?? resolveAgentConfig(task.agent ?? "worker", team);
