import { WorkflowResult, WaveResult, TaskConfig, CostSummary } from "./types.js";
...
export function renderWaveSummary(wave: WaveResult): string {
...
export function generateLogMarkdown(workflowDir: string, result: WorkflowResult): string {
  ...
  for (const task of result.tasks) {
    ...
    if (task.cost !== undefined) lines.push(`  Cost: ${formatCost(task.cost)}`);
    if (task.durationMs !== undefined) lines.push(`  Duration: ${formatDuration(task.durationMs)}`);
    if (task.files && task.files.length > 0) lines.push(`  Files: ${task.files.join(", ")}`);
    if (task.result) lines.push(`  Result: ${task.result}`);
    if (task.errors && task.errors.length > 0) lines.push(`  Errors: ${task.errors.join("; ")}`);
  }
